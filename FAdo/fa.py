...
from FAdo.unionFind import UnionFind
import FAdo.graphs
...
class SemiDFA(Drawable):
    ...
    def dotFormat(self, size="20,20", direction="LR", sep="\n"):
        ...
        for si in range(len(self.States)):
            ...
            s += "node [shape = point]; \"dummy{0:s}\"{1:s}".format(graphvizTranslate(sn), sep)
            s += self.dotDrawState(si)
            s += "\"dummy{0:s}\" -> \"{1:s}\";{2:s}".format(sn, graphvizTranslate(str(self.States[si])), sep)
        for si in range(len(self.States)):
        ...
        if sti >= len(self.States):
            ...
            for s in self.Final:
                if sti < s:
                    toDel.add(s)
                    toAdd.add(s - 1)
            self.Final = self.Final - toDel | toAdd
...
class OFA(FA):
    ...
    @abstractmethod
    def stateChildren(self, _,_a=None):
...
class NFA(OFA):
    ...
    def dotFormat(self, size="20,20", direction="LR", sep="\n", strict=False, maxLblSz=6):
        ...
        for si in o.Initial:
            ...
            s += o.dotDrawState(si)
            s += "\"dummy{0:s}\" -> \"{1:s}\";{2:s}".format(sn,graphvizTranslate(str(o.States[si])), sep)
    ...
    #todo: change program to avoid use of stateIndex()
    ...
    def HKequivalence(self, other):
        ...
        while stack:
            (p, q) = stack.pop()
            if (p <= self.Final) != (set([i - n for i in q]) <= other.Final):
                return False
...
class DFA(OFA):
    ...
    def succintTransitions(self):
        ...
        for k in foo:
            ...
            for c in cs[1:]:
                s += ", %s" % str(c)
            lst.append((str(self.States[k[0]]), s, str(self.States[k[1]])))
        return lst
    ...
    def sop(self, other):
        ...
        .. seealso:: Nelma Moreira, Giovanni Pighizzini, and Rogério Reis. Universal disjunctive concatenation and star. In Jeffrey Shallit and Alexander Okhotin, editors, Proceedings of the 17th Int. Workshop on Descriptional Complexity of Formal Systems (DCFS15), number 9118 in LNCS, pages 197--208. Springer, 2015.
    ...
    def orderedStrConnComponents(self):
        ...
        :rtype: list"""
        def _topOrder(i, j):
            if l1[str(j)] in l[l1[str(i)]]:
                return -1
            elif l1[str(i)] in l[l1[str(j)]]:
                return 1
            else:
                return 0
    ...
    def __and__(self, other,complete=False, trim=True):
        ...
        .. note:: This version does not use the program method
    ...
    def productSlow(self, other, complete=True):
        ...
        :param other: the other DFA
        :param bool complete: evaluate program as a complete DFA
        :rtype: DFA"""
        NSigma = self.Sigma.union(other.Sigma)
