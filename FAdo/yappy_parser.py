...
class CFGrammar(object):
    ...
    def DERIVE_T(self):
        ...
        while e:
            e = 0
            for s in self.nonterminals:
                for i1 in self.ntr[s]:
                    r = self.rules[i1][1]
                    if not r:
                        continue
                    for i1 in range(len(r)):
                        if r[i1] in self.terminals:
                            if i1 < len(r) - 1:
                                if r[i1 + 1] in self.derive_ter:
                                    if s not in self.derive_ter:
                                        self.derive_ter[s] = Set([])
                                    if self.derive_ter[s].s_append(r[i1]):
                                        e = 1
                                break
                            else:
                                if s not in self.derive_ter:
                            ...
                            """ non-terminal"""
                            if r[i1] in self.derive_ter:
                                if s not in self.derive_ter:
                                    self.derive_ter[s] = Set([])
                                if self.derive_ter[s].s_extend(self.derive_ter[r[i1]]) == 1:
                                    e = 1
                            if i1 > 0 and self.nullable[r[i1]]:
                                continue
                            else:
                                break
...
class Yappy_grammar(Yappy):
    ...
    def RULERule(self, arg, context):
        ...
        def grule(self, l):
            if not l: return (lhs, [], EmptySemRule)
