...
from src.MOEA.pymoo.NSGA3.PymooNsga3Modified import NSGA3


class EApymooNSGAIII:
    ...
    class MyCrossover(Crossover):
        ...
        def do(self, problem, pop, parents, **kwargs):
            ...
            # take the first 100 to avoid duplicates
            X = X[0:92]
    ...
    def run(self):
        flog = open('results/' + self.program + '/NSGAIII/' + self.objectives + "/" + str(self.n_run) + '_LogN_' + self.program + '.txt', 'w', 1)
        sys.stdout = flog
        start = time.perf_counter()
        start_p = time.process_time()
        ref_dirs = get_reference_directions("das-dennis", 3, n_partitions=12)
        algorithm = NSGA3(pop_size=self.n_pop,
                          ref_dirs=ref_dirs,
        ...
        f = open('results/' + self.program + '/NSGAIII/' + self.objectives + "/" + str(self.n_run) + 'RunNSGAIII' + self.program + '.txt', 'w')
        ...
        ff = open('results/' + self.program + '/NSGAIII/' + self.objectives + "/" + str(self.n_run) + '_EAfrontNSGAIII_' + self.program + '.pkl',
                  'wb')
