...
import numpy as np
import time
# from src.MOEA.pymoo.genetic_algorithm import GeneticAlgorithm
...
class RankAndCrowdingSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, D=None, **kwargs):
        ...
        infeasible_indexis = list()
        for i in range(len(pop)):
            if pop[i].X[3] == "infeasible":
                infeasible_indexis.append(i)
        if len(infeasible_indexis) > 0:
            pop = np.delete(pop, infeasible_indexis)
            self.removed_count += len(infeasible_indexis)

        if not self.gen % 9:
            print("Removed: " + str(self.removed_count) + "\n")
        ...
        # print("Time survival " + str(datetime.timedelta(seconds=round(time.perf_counter() - start_time))))
        # print("Time survival " + str(round(time.perf_counter() - start_time, 1)))
        # print("Population sizes after replacement: ", end="")
        # for indv in pop[survivors]:
