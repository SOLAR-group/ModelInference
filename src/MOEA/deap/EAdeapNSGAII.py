...
class EAdeapNSGAII:
    ...
    # Individuals made of a single trace
    ...
    def evaluateI(self, individual):  # UnRec, Size, UnObs
        if self.objectives == "UnRec-Size-UnObs":
            ...
            return self.fsm.UnRec(individual), len(individual[0].States)
        elif self.objectives == "UnObs-Size":
            return self.fsm.UnObs(individual[0]), len(individual[0].States)
    ...
    def compute(self):
        flog = open('data/'+self.product+'/NSGAII/' + str(self.n_run) + '_LogN_' + self.product + '.txt', 'w', 1)
        sys.stdout = flog
        ...
        for g in range(1, 101):
            ...
            if self.objectives != "UnRec-Size":
                ...
                rem = list()
                for j in range(len(child)):
                    if child[j].fitness.values[index] > 10000:
                        rem.append(j)
                rem.sort(reverse=True)
                for i in rem:
                    del child[i]
            ...
            start = time.perf_counter()
            # Reinsertion strategy NsgaII
            all_p = pop + child

            pop = tools.selNSGA2(all_p, self.n_pop)
        ...
        f = open('data/' + self.product + '/NSGAII/' + str(self.n_run) + 'RunNSGAII' + self.product + '.txt', 'w')
        ...
        if self.objectives != "UnRec-Size":
            ...
            for j in range(len(front)):
                if front[j].fitness.values[index] > 500 or len(front[j][0].States) > 500:
                    rem.append(j)
            rem.sort(reverse=True)
            for h in rem:
                del front[h]
        ...
        ff = open('data/' + self.product + '/NSGAII/' + str(self.n_run) + '_EAfrontNSGAII_' + self.product + '.pkl',
                  'wb')
        pickle.dump(mh_p, ff)
        ff.close()
